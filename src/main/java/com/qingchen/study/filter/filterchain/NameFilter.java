 ...
 **/
public class NameFilter implements RuleFilter<Rule> {

    @Override
    public List<Rule> filterList(List<Rule> list, FilterChain chain, int pos) {
        System.out.println("执行了Name");
        List<Rule> collect = list.stream().filter(rule -> rule.getName().length() > 3).collect(Collectors.toList());
