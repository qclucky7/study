...
import com.qingchen.study.vlife.HttpClientManager;
...
public class HttpClientUtils {
	...
	// 默认的连接配置
	private static final RequestConfig defaultRequestConfig = RequestConfig.custom().setConnectTimeout(3000).setSocketTimeout(10000).build();
	 ...
	 * @param reqEntity
	 * @param headers
	 * @param requestConfig 连接请求配置
	 * @return byte[]
	 * @date 2016-8-14
	 */
	public static byte[] doPost(String url, HttpEntity reqEntity, Header[] headers, RequestConfig requestConfig) throws IOException {
		CloseableHttpResponse response = null;
		long startTime = System.currentTimeMillis();
		int status = 0;
		try {
			...
			request.setConfig(requestConfig == null ? defaultRequestConfig : requestConfig);
			request.setEntity(reqEntity);
			response = client.execute(request, new BasicHttpContext());
			status = response.getStatusLine().getStatusCode();
			if (HttpStatus.SC_OK == status || HttpStatus.SC_PARTIAL_CONTENT==status) {
				byte[] result = EntityUtils.toByteArray(response.getEntity());
				logger.info("doPost result = {}", new String(result));
				return result;
			} else {
				throw new RuntimeException("receive code:" + status);
			}
			...
			long endTime = System.currentTimeMillis();
			logger.info("post end url:{} time:{}", url, System.currentTimeMillis()-startTime);
//			MonitorService.getInstance().putHttpClient(0L, url, startTime, endTime, null, null, status);
			if(response!=null) {
				response.close();
			}
		}
